...
The maximum integer value that can be held by `tinytcp_common_ssize_t` (which can be found in `"common.h"`; see *Types*)
...
The constant that represents an invalid `tinytcp_common_socket_t` socket (which can be found in `"common.h"`; see *Types*).
...
IMPORTANT: This function must be called only *once*.
