...
- `int domain`: The domain of the socket. Must match *exactly* to the value used when creating the socket.
...
Windows Return: A new socket to the client on success or `TINYTCP_COMMON_INVALID_SOCKET` (which can be found in `"common.h"`; see `common.md:Macros:Constant definition macros`) on failure.
POSIX Return: A new socket to the client on success or `TINYTCP_COMMON_INVALID_SOCKET` (which can be found in `"common.h"`; see `common.md:Macros:Constant definition macros`) on failure.
